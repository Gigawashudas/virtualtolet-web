    ...
    try {
      ...
      if (form.propertyType !== "Garage") {
        ...
        const { data: unit, error: unitError } = await supabase
          ...
          .insert({
            ...
            size_sqft: form.propertyType === "Apartment" && form.size ? Number(form.size) : null,
            description: null,
          })
          .select("id")
          .single();
      ...
      const details = {
        ...
        gate: {
          ...
          open_from: form.gateAccess === "Fixed hours" ? form.gateOpenFrom || null : null,
          open_to: form.gateAccess === "Fixed hours" ? form.gateOpenTo || null : null,
        },
        ...
        parking: {
          bike: {
            ...
            monthly_charge: form.bikeParking && form.bikeParkingCharge === "Extra charge" && form.bikeParkingChargeAmount.trim() ? Number(form.bikeParkingChargeAmount) : null,
          },

          car: {
            available: form.carParking,
            charge_type: form.carParking ? form.carParkingCharge : null,
            monthly_charge: form.carParking && form.carParkingCharge === "Extra charge" && form.carParkingChargeAmount.trim() ? Number(form.carParkingChargeAmount) : null,
          },
        },
      ...
      <main className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-4xl px-6 py-12 sm:px-8 lg:px-10">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            <button type="button" onClick={handleSubmit} disabled={submitting} className="h-12 rounded-lg bg-brand-green px-7 text-sm font-bold text-white transition hover:opacity-90">
              ...
              <div className="grid gap-6">
                ...
                {form.propertyType === "Garage" && (
                  <>
                    ...
                    <Field label="Size">
                      <div className="relative">
                        <input required type="number" min="0" value={form.size} onChange={(event) => updateField("size", event.target.value)} placeholder="e.g. 150" className={`${inputClass} pr-16`} />
              ...
              <div>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  {photos.map((photo, index) => (
                    <div key={photo.id} className="group relative overflow-hidden rounded-lg border border-border bg-surface">
                      <img src={photo.url} alt={`Listing photo ${index + 1}`} className="h-44 w-full object-cover" />
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  {photos.length < MAX_PHOTOS && (
                    <label className="flex h-32 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border-strong text-text-secondary transition hover:border-hover-border hover:bg-hover-background hover:text-hover-text">
                  ...
                  {videos.length < MAX_VIDEOS && (
                    <label className="flex h-32 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border-strong text-text-secondary transition hover:border-hover-border hover:bg-hover-background hover:text-hover-text">
                      ...
                      <span className="text-sm font-bold">Add videos</span>
